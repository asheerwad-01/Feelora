...
export function LibraryPanel() {
  ...
  const sources = useMemo(() => {
    ...
    if (connectedProviders.appleMusic) list.push('Apple Music');
    if (connectedProviders.youtubeMusic) list.push('YouTube Music');
                    ...
                    {song.provider && song.provider !== 'demo' && (
                      <div className="absolute bottom-0 right-0 w-3.5 h-3.5 rounded-tl-md bg-black/80 flex items-center justify-center border-l border-t border-white/5">
                        ...
                        {song.provider === 'apple-music' && (
                          <svg width="7" height="7" viewBox="0 0 24 24" fill="#FF2D55"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h6V3h-8z"/></svg>
                        )}
                        {song.provider === 'youtube-music' && (
                          <svg width="7" height="7" viewBox="0 0 24 24" fill="#FF0000"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/></svg>
                        )}
                      </div>
                    )}
                  </div>
